...
public class UpTopActivity extends AppCompatActivity {
    ...
    public static void navToChat(Context context,  String type){
        Intent intent = new Intent(context, UpTopActivity.class);
        intent.putExtra("topicId", type);
        context.startActivity(intent);
    }
    public void bindview()
{
    type=getIntent().getStringExtra("topicId");
    setContentView(setInflateId());
    gridView = (NoScrollGridView)findViewById( R.id.gridView);
    toolbar = (Toolbar) findViewById( R.id.toolbar);
    et_title=(EditText)findViewById(R.id.et_title);
    et_title.setText("#"+ Constants.topic[Integer.parseInt(type)]+"#");
        ...
        public View getView(final int position, View convertView, ViewGroup parent) {
          ...
          /*  } else {
            ...
            if (position == this.picList.size()) {
                holder.image.setImageBitmap(BitmapFactory.decodeResource(UpTopActivity.this.getResources(), R.drawable.icon_addpic));
                ...
                Glide.with(UpTopActivity.this).load((String) this.picList.get(position)).into(holder.image);
            }
            ...
            holder.image.setOnClickListener(new OnClickListener() {
                public void onClick(View view) {
                    if (position == GridAdapter.this.picList.size()) {
                        PhotoPickerActivity.showWithParams(UpTopActivity.this, true, 1, 9 - GridAdapter.this.picList.size());
                    }
                }
            });
            return convertView;
    ...
    void sendShare() {
        BooDialog dialog = new BooDialog.BooBuilder(UpTopActivity.this).title("发布分享").progressMode().setMessage("请稍候...").show(UpTopActivity.this.getFragmentManager());
        ...
        if (TextUtils.isEmpty(text)||TextUtils.isEmpty(title)) {
         ...
         if (this.picPaths == null || this.picPaths.size() <= 0) {
            ...
            Intent intent = new Intent(this, UpTopicService.class);
  ...
  public void  showToast(String s){
    ...
    private void uploadShare(String text, String title,String picJsonArray, final BooDialog dialog) {
        ...
        UploadImp.uploadTopicblog(token,id,"",type,text, picJsonArray, new ActionCallback<String>() {
            @Override
